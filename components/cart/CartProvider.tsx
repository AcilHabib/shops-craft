...
const CartProviderContext = ({ children }: {children: React.ReactNode}) => {
    ...
    useEffect(() => {
        ...
        const createCartIfNotExists = async () => {
            ...
            if (isCartEmpty) {
                ...
                const res = await fetch(`http://localhost:3000/api/carts/cart?cartId=${localStorage.getItem('cart')?.slice(1, -1)}`);
                if (!res.ok) {
    ...
    const createCartItem = async (product: Product): Promise<CartItem> => {
        ...
        const res = await fetch(`http://localhost:3000/api/carts/items?cartId=${cartId?.slice(1, cartId.length - 1)}&productId=${product.id}`, {
            method: 'POST',
            headers: {
    ...
    const handleUpdateItemQuantity = async (event: React.FormEvent, item: CartItem, type: string) => {
        ...
        const res = await fetch(`http://localhost:3000/api/carts/items?id=${item.id}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json'
          },
